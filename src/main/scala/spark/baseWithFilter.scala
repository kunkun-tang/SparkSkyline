...
object BaseWithFilter {
  ...
  def main(args: Array[String]) {
    ...
    val (inflInsts, candObjs) = individual.map(part => {
    	...
    	val firstPhaseQuery = new OptimizedQueryNoWRTreeSpark(part._1, itemMap, outputLists);
        val results = firstPhaseQuery.compProb();
        results;
    ...
    end("firstPhase Query and Output")
    ...
    println("candidate Instance length = "+ candInsts.count.toInt + "-- remainInsts length = "+ remainInsts.count.toInt);
    /*
    ...
    end("Secon Phase Query and Output")
